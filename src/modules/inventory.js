        ...
        exportPDF() {
            ...
            div.innerHTML = `
                <div class="text-center mb-6">
                    ...
                    <p class="text-gray-400">${term ? `Tabela de Preços: "${esc(term)}"` : 'Tabela Geral de Preços'}</p>
                </div>
                <table class="w-full text-sm">
                    <thead><tr class="text-left border-b border-gray-700"><th class="py-2">Produto</th><th class="py-2 text-center">Qtd</th><th class="py-2 text-right">Preço Venda</th></tr></thead>
                    <tbody>${filtered.map(p => `<tr class="border-b border-gray-800"><td class="py-2">${esc(p.name)}</td><td class="py-2 text-center">${p.qty}</td><td class="py-2 text-right font-bold">${App.utils.formatMoney(p.price)}</td></tr>`).join('')}</tbody>
                </table>
                <div class="mt-8 text-center text-xs text-gray-500">Gerado em ${new Date().toLocaleString('pt-BR')}</div>`;
            window.html2pdf().set({ margin: 10, filename: `Tabela_Precos_${term || 'Geral'}.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, backgroundColor: '#111827' }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } }).from(div).save();
        },
